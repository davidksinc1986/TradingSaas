...
document.getElementById('run-form')?.addEventListener('submit',async(e)=>{e.preventDefault();try{const payload=buildRunPayload(e.target,{singleConnector:false});await api('/api/strategies/run',{method:'POST',body:JSON.stringify(payload)});setRunFeedback(t('strategy_executed','Estrategia ejecutada correctamente con validación de símbolos y riesgo.'));refreshDashboard();}catch(err){setRunFeedback(parseApiError(err),'error');}});
