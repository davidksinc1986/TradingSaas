...
function selectedRunPlatformPreference(){return String(document.getElementById('run-platform-filter')?.value||'').trim().toLowerCase();}
...
function selectedMarketType(){return selectedMarketTypes()[0]||'spot';}function selectedRunMarketTypePreference(){return String(document.getElementById('run-market-type-filter')?.value||'').trim().toLowerCase();}function resolveEffectiveRunMarketType(){return selectedRunMarketTypePreference()||selectedMarketType()||'spot';}function refreshRunPlatformFilter(){const select=document.getElementById('run-platform-filter');if(!select)return;const enabledConnectors=(DASHBOARD_CACHE.connectors||[]).filter(c=>c&&c.is_enabled);const platforms=[...new Set(enabledConnectors.map((connector)=>String(connector.platform||'').toLowerCase()).filter(Boolean))].sort((a,b)=>a.localeCompare(b));const previous=selectedRunPlatformPreference();select.innerHTML=`<option value="">${isEnglishDashboard()?'All active platforms':'Todas las plataformas activas'}</option>`+platforms.map((platform)=>`<option value="${platform}">${String(platform).toUpperCase()}</option>`).join('');if(previous&&platforms.includes(previous))select.value=previous;}function renderRunSelectionSummary(connectors){const node=document.getElementById('run-selection-summary');if(!node)return;const platform=selectedRunPlatformPreference();const market=selectedRunMarketTypePreference();const labels=[];labels.push(platform?`Plataforma ${platform.toUpperCase()}`:'todas las plataformas');labels.push(market?`mercado ${market}`:'mercado detectado');labels.push(`${connectors.length} conector(es) visible(s)`);node.textContent=`Filtra primero la app y el mercado para trabajar sobre el conector correcto. Vista actual: ${labels.join(' · ')}.`;}function syncRunMarketTypeUI(){const marketSelect=document.getElementById('run-market-type-filter');const runSelect=document.getElementById('run-connector-select');if(!marketSelect||!runSelect)return;refreshRunPlatformFilter();const enabledConnectors=(DASHBOARD_CACHE.connectors||[]).filter(c=>c&&c.is_enabled);const marketPreference=selectedRunMarketTypePreference();const platformPreference=selectedRunPlatformPreference();const visibleConnectors=enabledConnectors.filter((connector)=>{const connectorPlatform=String(connector.platform||'').toLowerCase();const connectorMarket=String(connector.market_type||'spot').toLowerCase();return (!platformPreference||connectorPlatform===platformPreference)&&(!marketPreference||connectorMarket===marketPreference);});const previousSelection=selectedRunConnectorIds();runSelect.innerHTML=visibleConnectors.map(c=>`<option value="${c.id}" data-market-type="${c.market_type||'spot'}" data-platform="${c.platform||''}">${c.label} (${c.platform} - ${c.market_type||'spot'} - ${c.mode||'paper'})</option>`).join('');Array.from(runSelect.options).forEach((option)=>{const id=Number(option.value);option.selected=previousSelection.includes(id)||(visibleConnectors.length===1&&id===Number(visibleConnectors[0]?.id));});if(!runSelect.options.length){runSelect.innerHTML=`<option value="" disabled>${t('connector_for_market_missing','No hay conectores para ese mercado')}</option>`;}renderRunSelectionSummary(visibleConnectors);renderStrategySelect();renderStrategyGuidance();updateSymbolSourceUI();loadSymbolCatalog();}function strategyGuidePreset(slug,marketType){const presets={spot:{take_profit_value:1.5,stop_loss_value:1.0,trailing_stop_value:0.8,leverage_profile:'none',max_open_positions:1},futures:{take_profit_value:1.2,stop_loss_value:0.6,trailing_stop_value:0.4,leverage_profile:'balanced',max_open_positions:1}};const base={...(presets[marketType]||presets.spot)};if(slug==='ema_rsi_adx_stack'){return {...base,take_profit_value:1.8,stop_loss_value:0.8,trailing_stop_value:0.55,max_open_positions:2};}if(slug==='volatility_breakout'||slug==='momentum_breakout'){return {...base,take_profit_value:1.2,stop_loss_value:0.5,trailing_stop_value:0.4,leverage_profile:'balanced'};}return base;}function applyRunMarketPreset(){const form=document.getElementById('run-form');if(!form)return;const marketType=resolveEffectiveRunMarketType();const slug=String(document.querySelector('select[name="strategy_slug"]')?.value||'');const preset=strategyGuidePreset(slug,marketType);const tp=form.querySelector('input[name="take_profit_value"]');const sl=form.querySelector('input[name="stop_loss_value"]');const trailing=form.querySelector('input[name="trailing_stop_value"]');const leverage=form.querySelector('input[name="leverage_profile"]');const maxPos=form.querySelector('input[name="max_open_positions"]');if(tp)tp.value=Number(preset.take_profit_value);if(sl)sl.value=Number(preset.stop_loss_value);if(trailing)trailing.value=Number(preset.trailing_stop_value);if(leverage)leverage.value=String(preset.leverage_profile||'none');if(maxPos)maxPos.value=Number(preset.max_open_positions||1);}
...
function visibleRunConnectorIds(){return Array.from(document.getElementById('run-connector-select')?.options||[]).map((option)=>Number(option.value)).filter((value)=>Number.isFinite(value)&&value>0);}
function activeConnectorForMarket(){const connectors=Array.isArray(DASHBOARD_CACHE.connectors)?DASHBOARD_CACHE.connectors:[];const visibleIds=visibleRunConnectorIds();const selected=selectedRunConnectorIds().filter((id)=>visibleIds.includes(id));if(selected.length){const picked=connectors.find((c)=>c.id===selected[0]);if(picked)return picked;}if(visibleIds.length===1){return connectors.find((c)=>c.id===visibleIds[0])||null;}return null;}
...
function setRunWizardStep(step){const safe=Math.max(1,Math.min(3,Number(step)||1));document.querySelectorAll('[data-wizard-step]').forEach((btn)=>btn.classList.toggle('active',Number(btn.dataset.wizardStep)===safe));document.querySelectorAll('[data-wizard-content]').forEach((pane)=>pane.classList.toggle('active',Number(pane.dataset.wizardContent)===safe));const prev=document.getElementById('wizard-prev-inline');const next=document.getElementById('wizard-next-inline');const submit=document.getElementById('save-bot-btn');if(prev)prev.disabled=safe===1;if(next){next.hidden=safe===3;next.textContent='Siguiente';}if(submit)submit.classList.toggle('pulse-attention',safe===3);document.getElementById('run-form')?.setAttribute('data-step',String(safe));}
function initInlineRunWizard(){setRunWizardStep(1);document.querySelectorAll('[data-wizard-step]').forEach((btn)=>btn.addEventListener('click',()=>setRunWizardStep(Number(btn.dataset.wizardStep))));document.getElementById('wizard-prev-inline')?.addEventListener('click',()=>setRunWizardStep(Number(document.getElementById('run-form')?.dataset.step||1)-1));document.getElementById('wizard-next-inline')?.addEventListener('click',()=>{const current=Number(document.getElementById('run-form')?.dataset.step||1);if(current>=3){document.getElementById('run-form')?.requestSubmit();return;}setRunWizardStep(current+1);});}
