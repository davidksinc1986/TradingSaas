...
class BotSession(Base):
    ...
    trade_amount_mode: Mapped[str | None] = mapped_column(String(20), nullable=True)
    amount_per_trade: Mapped[float | None] = mapped_column(Float, nullable=True)
    amount_percentage: Mapped[float | None] = mapped_column(Float, nullable=True)
    min_ml_probability: Mapped[float] = mapped_column(Float, default=0.55)
    use_live_if_available: Mapped[bool] = mapped_column(Boolean, default=True)
