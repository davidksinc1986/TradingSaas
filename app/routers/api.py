    ...
    for session in sessions:
        try:
            ...
            payload.append({
                ...
                "max_open_positions": max(int(getattr(session, "max_open_positions", 10) or 10), 1),
...
def copy_bot_session(session_id: int, payload: BotSessionCopyPayload, db=Depends(get_db), user=Depends(current_user)):
    try:
        ...
        cloned = BotSession(
            ...
            max_open_positions=max(int(getattr(source, "max_open_positions", 10) or 10), 1),
...
def apply_strategy_template(template_id: int, payload: StrategyTemplateApplyPayload, db=Depends(get_db), user=Depends(current_user)):
    ...
    session = BotSession(
        ...
        max_open_positions=max(int(cfg.get("max_open_positions", 10) or 10), 1),
